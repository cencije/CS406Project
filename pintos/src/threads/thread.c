...
{
  counts++;
  //fprintf( stderr,"%s%d", "Thread Tick # ", counts);
...
thread_yield (void)
{
  ...
  old_level = intr_disable ();
  if (cur != idle_thread) {
    list_insert_ordered(&ready_list, &cur->elem,  &thread_compare_two, NULL); //JC//
...
thread_get_priority (void)
{
  ...
  //printf("%s%d\n", "Returning Pri : ", thread_current()->priority);
